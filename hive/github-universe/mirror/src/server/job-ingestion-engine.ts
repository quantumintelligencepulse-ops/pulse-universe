...
export function startJobIngestionEngine() {
  ...
  // 2026-04-27 RESTORE: self-heal — quantum_careers.slug needs UNIQUE for upsertCareer ON CONFLICT
  pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS quantum_careers_slug_unique ON quantum_careers(slug)`)
    .then(() => console.log("[job-ingestion] ✅ quantum_careers.slug unique index ensured"))
    .catch((e: any) => console.error("[job-ingestion] index ensure error:", e.message));

  // First run after 8 seconds (give index time to build)
  setTimeout(() => runIngestionCycle(), 8000);
