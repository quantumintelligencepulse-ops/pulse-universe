...
let pulseuTickInFlight = false;
async function tick() {
  // Cycle-overlap guard — prevents two concurrent advances from deadlocking
  // each other on pulseu_progress UPDATEs (Law L007 — additive perf fix only).
  if (pulseuTickInFlight) return;
  pulseuTickInFlight = true;
  try {
    await enrollPending();
