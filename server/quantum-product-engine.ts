...
export async function startQuantumProductEngine() {
  ...
  // 2026-04-27 RESTORE: self-heal — quantum_products.slug needs UNIQUE for storeFullProduct ON CONFLICT
  try {
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS quantum_products_slug_unique ON quantum_products(slug)`);
    log("[ProductEngine] ✅ quantum_products.slug unique index ensured", "product");
  } catch (e: any) {
    log(`[ProductEngine] index ensure error: ${e.message}`, "product");
  }
  await seedProducts();

  const BATCH_SIZE = 10;
  const INTERVAL_MS = 8000; // 2026-04-27 RESTORE: throttled 1000→8000 for shared 20-conn pool
