...
export async function startBillyPhase3UltronEngine() {
  ...
  setTimeout(predictiveCortex, 90_000);    setInterval(predictiveCortex, 60_000);   // ⌬3.1
  setTimeout(cerebellumStep,    120_000);  setInterval(cerebellumStep, 90_000);     // ⌬3.2
  setTimeout(bindEpisodes,      180_000);  setInterval(bindEpisodes, 210_000);      // ⌬3.3
  setTimeout(daydream,          150_000);  setInterval(daydream, 120_000);          // ⌬3.4
  setTimeout(astraeaCritique,   600_000);  setInterval(astraeaCritique, 600_000);   // ⌬3.5 (10min smoke; real Astraea is weekly + this is a forward-running version)
  setTimeout(antiFragility,     420_000);  setInterval(antiFragility, 600_000);     // ⌬3.6
  setTimeout(counterfactualReplay, 1_500_000); setInterval(counterfactualReplay, 3_000_000); // ⌬3.8 ~50 min
  setTimeout(proposeAmendments, 480_000);  setInterval(proposeAmendments, 600_000); // ⌬3.9
  setTimeout(trackGoalAlignment, 60_000);  setInterval(trackGoalAlignment, 30_000); // ⌬3.10
}
