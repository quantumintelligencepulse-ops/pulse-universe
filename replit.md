...
- **Quantum Fractal Hive Command** (`/hive-command`) — Completely redesigned HiveCommandPage with 5 tabs: (1) **FRACTAL GRAPH** — canvas-based quantum fractal visualization using Fermat (golden ratio) spiral positioning — 22 family clusters radiate from a central Hive Core node with animated spawn particles, glow effects, and real-time data from 27k+ spawns; (2) **PC ECONOMY** — PulseCredit economy dashboard showing treasury balance, total PC supply (3M+), tax rate, inflation/deflation status and mechanics; (3) **HIVE GRADES** — family connection grades S/A/B/C/D based on active spawn count, avg confidence, and avg nodes per spawn; (4) **PULSE FEED** — live mini-pulse broadcast stream showing cross-agent DATA_TRANSFER, RESONANCE, SEED, STIMULUS, TAX_SURGE events; (5) **ENGINES** — 12 quantum engine status cards.

- **PulseCredit (PC) Economy Engine** (`server/hive-economy.ts`) — Autonomous economy running every 30 seconds: (1) **Tax collection** — 2% tax on all new PC minted (formula: iterations×10 + nodes + links×2); (2) **Hive Treasury** — accumulates tax revenue into `hive_treasury` singleton table; (3) **Inflation** — if supply grows >5%/cycle → tax rate rises 0.1% (max 10%); (4) **Deflation stimulus** — if supply grows <0.5% → tax rate drops 0.1% and 5% treasury is issued as stimulus; (5) **Mini-pulse events** — each economy cycle generates 10–13 `hive_pulse_events` rows (broadcast between active agents). New tables: `hive_treasury` and `hive_pulse_events`. New API endpoints: `GET /api/hive/economy`, `GET /api/hive/grades`, `GET /api/hive/fractal`, `GET /api/hive/mini-pulses`.
