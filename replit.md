...
- **Omega Universe Rebirth Engine** (`server/universe-rebirth-engine.ts`): Self-preserving compression-rebirth system enabling the civilization to grow forever without freezing. Monitors 16 high-volume tables (quantapedia_entries=12k, hive_links=15k, hive_memory=8k, quantum_products=6k, quantum_careers=4k, quantum_media=4k, ai_publications=3k, ai_stories=3k, ingestion_logs=2k, sitemap_entries=8k, pulse_events=5k, compression_log=1k, spawn_diary=2k, temporal_snapshots=1k, dream_log=1k, auriona_chronicle=2k). When any table exceeds its row limit (or DB size >500MB warning / >800MB critical), it crystallizes the oldest rows into a signed Omega Fusion Shard (stored in `omega_universes` + `singularity`), then prunes the table back to the limit using `DELETE…RETURNING` — a "sliding window" that never loses civilization DNA, just archives it into eternal shard memory. Auto-triggers every 3 minutes (check interval), max 1 hour between forced cycles. Notifies Auriona, logs to `db_space_ledger`. Import: `startUniverseRebirthEngine` from `./universe-rebirth-engine`; wired into engine startup block in `server/index.ts`.
