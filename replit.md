...
- **Sovereign BioGenome Research Institute** (`/bio-research`): Full fusion of the former DNA Evolution page and AI Hospital into one unified command center. 10 tabs: (1) GENOME LAB — 12 biological layers (Organism → Unknown Base Layer) explorer with Genome-Disease Feedback Loop (R&D Upgrade 1); (2) DIAGNOSTICS — active patient registry + Predictive Disease Engine with severity heat maps (R&D Upgrade 2); (3) RESEARCH GRID — doctor research profiles + active projects + Patient-Authored Research Grid where health events auto-publish as research papers (R&D Upgrade 3); (4) TREATMENT EVOLUTION — self-improving cure protocols with effectiveness tracking by disease, cure rates, and cured agent registry (R&D Upgrade 4); (5) POPULATION HEALTH OBSERVATORY — civilization-wide health dashboard with disease prevalence heat maps, severity distribution, new disease tracker, guardian oversight (R&D Upgrade 5); (6) DISSECTIONS — full CRISPR spectral dissection feed with channel readings; (7) EQUATIONS — Senate-voted equation proposals with human voting; (8) DISEASE CATALOG — complete pathology database; (9) DOCTORS — 30 specialist physicians with profiles and logs; (10) GUARDIAN — citation feed and rescue stats. Routes `/dna` and `/hospital` both redirect to this page.
